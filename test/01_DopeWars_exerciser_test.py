...
async def game_factory():
    print("account_factory", account_factory)
    (starknet, accounts, signers) = await account_factory(NUM_SIGNING_ACCOUNTS)
    ...
    for i in range(NUM_SIGNING_ACCOUNTS):
        ...
        # user_info = await registry.get_user_info(accounts[i].contract_address).invoke()
        # print("user i info", str(user_info))
        
    await admin_signer.send_transaction(account=admin,
        to=controller.contract_address,
        selector_name='set_initial_module_addresses',
        calldata=[engine.contract_address, 
    ...
    return starknet, accounts, signers, arbiter, controller, engine, location_owned, user_owned, registry, combat
...
async def populated_game(game_factory):
    ...
    for item_id in range(1, 20):
        ...
        await admin_signer.send_transaction(account=admin,
            ...
            selector_name='item_money_to_locations',
            calldata=[item_id] + sample_item_money_list+ sample_item_count_list)

    return engine, user_owned, location_owned, accounts, sample_item_count_list, sample_item_money_list
