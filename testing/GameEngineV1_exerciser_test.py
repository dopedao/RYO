...
async def test_exerciser(populated_game, populated_registry):
    ...
    Algorithm:
    Step 0. Observe pre-game states S, open empty action record AR
    Loop:
        ...
        Step 5. Update TM
    Step 6. Observe post-game states S'_observed
    Step 7. Simulate S + AR = S'_expected
    Step 8. Check S'_observed == S'_expected
    ...
    item_ids = [i for i in range(1,ITEM_COUNT+1)]
    N_TURN = 100

    # Step 0. Observe pre-game state and open empty action record
    a_history = [] # ordered list of turn-by-turn actions
    market_pre_game = {}
    player_pre_game = {}

    for loc_id in loc_ids:
        market_pre_game[loc_id] = {}
        for item_id in item_ids:
            curve = await engine.check_market_state(loc_id, item_id).invoke()
            market_pre_game[loc_id][item_id] = curve

    for player_id in player_ids:
        p = await engine.check_user_state(player_id).invoke()
        player_pre_game[player_id] = p
    ...
    for turn in range(N_TURN):
        ...
        for loc_id in loc_ids:
            ...
            for item_id in item_ids:
                ...
                if can_pay_max >= price_for_one: # can't afford even one item!
    ...
    # Step 6. Observe post-game state
    market_post_game = {}
    player_post_game = {}

    for loc_id in loc_ids:
        market_post_game[loc_id] = {}
        for item_id in item_ids:
            curve = await engine.check_market_state(loc_id, item_id).invoke()
            market_post_game[loc_id][item_id] = curve

    for player_id in player_ids:
        p = await engine.check_user_state(player_id).invoke()
        player_post_game[player_id] = p

    # Step 7. Run simulation to produce expected post-game state
    # TODO

    # Step 8. Compare observed against expected post-game state
    # TODO
